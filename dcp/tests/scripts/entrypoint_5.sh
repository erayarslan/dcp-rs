...
check_cluster_init(){
	bash /opt/couchbase/bin/couchbase-cli cluster-init -c 127.0.0.1:${REST_PORT} \
      ...
      --index-storage-setting ${INDEX_STORAGE_SETTING} > /dev/null
  echo $?
}
...
/opt/couchbase/bin/cbimport json -c 127.0.0.1:${REST_PORT} --username ${USERNAME} --password ${PASSWORD} \
--bucket ${BUCKET_NAME} --dataset /opt/couchbase/samples/travel-sample.zip --format sample
